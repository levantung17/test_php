                        ...
                        <div class="panel-body">
                            <form role="form">
                                 ...
                                 <div class="form-group">
                                    ...
                                    $c_check="";
                                    foreach($_POST['txtxcheck']as $i_check){
                                        $c_check.=$i_check;
                                    }
                                    move_uploaded_file($_FILES['txtfile']['tmp_name'],'upload/'.FILES['txtfile']['name']);
                                    $se_user=$get_data->select_user($_POST["txtuser"]);
                                    if(isset($_POST['txtsub'])){
                                        echo "<script>alert('Bạn chx nhập đủ dữ liệu')</script>";}
                                        else{
                                            if($_POST['txtpass']!=$_POST['txtrepass']){
                                                echo "<script>alert('Hai mật khẩu ko trùng nhau')</script>";
                                            }
                                            else{
                                                $register_user+$get_data->register($_POST['txtuser'],$_POST['txtpass'],$_POST['txtadd'],
                                                $_FILES['txtfile']['name'],$_POST['txtgen'],$c_check,
                                                $_POST['txtemail']);
                                                if($register_user)echo "<script>alert('Thêm mới thành công')</script>";
